...
def EditDataframe(list_length, df_dict):
    #Index
    out_index = ["江ノ村・時間","江ノ村・連続","押ノ川・時間",
    "押ノ川・連続","久礼・時間","九礼・連続","窪川・時間",
    "窪川・連続","佐賀・時間","佐賀・連続","右山・時間",
    "右山・連続","山奈・時間","山奈・連続","宿毛・時間","宿毛・連続"]
    #Column
    out_column= []
...
def AdjustExcel(output_file):
    ...
    for col in my_sheet.columns:
        ...
        if column == "A":
            my_sheet.column_dimensions[column].width = 12.00
        else:
